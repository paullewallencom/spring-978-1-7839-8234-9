...
public class ApplicationContextRefreshedListener implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		User shameer = userService
				.createNewUser(new User(null, "Shameer Kunjumohamed", "sameerean", "password", new Date()));
		// userService.createNewUser(shameer);
		discoverProfileImageIfExists(shameer);

		User tarun = userService.createNewUser(new User(null, "Tarun Bhati", "tbhati", "password", new Date()));
		...
		taskService.createTask(new Task("Commit code changes", 5, "Open", shameer, new Date(), tarun,
				"Let's test and deploy on Staging Server"));
		taskService.createTask(
				new Task("Review code changes", 6, "Open", tarun, new Date(), shameer, "Please send your feedback"));
		taskService.createTask(new Task("Release project version", 3, "Open", tarun, new Date(), shameer,
				"All tested, verified, accepted"));
		taskService.createTask(new Task("Order Snacks", 9, "Closed", tarun, new Date(), shameer,
				"Order some samosas and juices from Haji Ali"));
